    ...
    public static void checkForUpdates(final String currentVersion, final Callback cb) {
        new Thread(() -> {
            ...
            try {
                ...
                if (latest == null) {
                    // Every release is marked pre-release (or none parse):
                    // report "no update" instead of nudging users onto the
                    // pre-release channel (the old code fell back to
                    // releases[0], the newest item, regardless).
                    cb.onResult(false, currentVersion, null, null);
                    return;
                }
